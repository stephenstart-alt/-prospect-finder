...
async function insertListingChanges(rows) {
  ...
  for (let i = 0; i < rows.length; i += 500) {
    const batch = rows.slice(i, i + 500);
    try {
      const res = await fetch(`${SB_API}/listing_changes`, {
        method: 'POST',
        headers,
        body: JSON.stringify(batch),
      });
      if (res.ok) {
        ...
        const err = await res.text();
        console.warn(`  ⚠ listing_changes batch error: ${err.substring(0, 200)}`);
      }
    } catch (e) {
      console.warn(`  ⚠ listing_changes network error: ${e.message}`);
    }
  }
  return success;
}
...
async function main() {
  ...
  for (const [listingId, listing] of allListings) {
    ...
    if (keys.length > 0) {
      ...
      // FIX: hoist drop and pct out of the inner if-block so they're in scope throughout
