...
// ── Targeting config — reads from env vars (set by GitHub Actions workflow inputs)
//    or falls back to defaults. Tweak the defaults here if running locally.
const TARGET_CONFIG = {
  epc_max_age_months: parseInt(process.env.EPC_MAX_AGE_MONTHS) || 18,
  min_price:          parseInt(process.env.MIN_PRICE)          || 0,
  max_price:          parseInt(process.env.MAX_PRICE)          || 99999999,
...
async function runTarget() {
  ...
  // ── 2. Pull listing_changes and market_status for those keys ──
